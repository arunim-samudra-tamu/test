$(document).ready( function() {
  ...
  var stackNums = [0,3,4,2,0];
  var startNums = [0,3,4,2,0];
  ...
  document.getElementById("canvas_1").addEventListener('mousemove', function() {
    ...
    for(var i = 0; i < circles.length; i++) {
      for(var j = 0; j < circles[i].length; j++) {
        if(stackNums[i] > j) circlesTotal++;
      }
    }

    if(circlesTotal % 2 == 1 || playing) {
      ...
      if(intersections.length > 0) {
        ...
        if(stackNums[0] == 0 && stackNums[1] == 0 && stackNums[2] == 0 && stackNums[3] == 0 && stackNums[4] == 0) {
          if($(`input:radio[name ='winCondition']:checked`).val() == "odd") {
            setWinnerOut($("#player1_total").val() % 2 == 1 ? "Blue" : "Red");
          }
          else {
            setWinnerOut($("#player1_total").val() % 2 == 1 ? "Red" : "Blue");
          }
        }
        togglePlayerOut();
  ...
  function placeCircles() {
    ...
    if(circlesTotal % 2 == 0 && !playing) {
      for(var i = 0; i < circles.length; i++) {
        for(var j = 0; j < circles[i].length; j++) circles[i][j].material = matRedAlpha;
      }
    }
    else if(!playing){
      for(var i = 0; i < circles.length; i++) {
        for(var j = 0; j < circles[i].length; j++) circles[i][j].material = matRed;
      }
    }
  }
