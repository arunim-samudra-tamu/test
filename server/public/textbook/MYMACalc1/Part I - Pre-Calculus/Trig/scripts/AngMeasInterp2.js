$(document).ready( function() {
  ...
  // RENDERER
  var renderer = new THREE.WebGLRenderer({canvas: document.getElementById("robo_2"), antialias: true});
  ...
  document.getElementById("robo_2").addEventListener('mousemove', function(event) {
    mouse.x = ((event.clientX - $("#robo_2").offset().left + $(window).scrollLeft()) / 50) / 3.5 - 1;
    mouse.y = -(((event.clientY - $("#robo_2").offset().top + $(window).scrollTop()) / 50)) / 3.5 + 1;
  ...
  document.getElementById("robo_2").addEventListener('mousedown', function() {
    if(!showing) {
  ...
  document.getElementById("robo_2").addEventListener('mouseup', function() {
    dragging = false;
  ...
  // BUTTONS
  var turning = 0;
  document.getElementById("leftButton_2").onmousedown = function() {
    if(!showing) {turning = -1; dragging = false;}
  };
  document.getElementById("leftButton_2").onmouseup = function() {
    if(!showing) {turning = 0}
  };
  document.getElementById("rightButton_2").onmousedown = function() {
    if(!showing) {turning = 1;  dragging = false;}
  };
  document.getElementById("rightButton_2").onmouseup = function() {
    if(!showing) {turning = 0}
  };
  ...
  document.getElementById("leftArmButton_2").addEventListener('click', function() {
    dragging = false;
    if(!showing) {
      if(lArmUp) {
        document.getElementById("leftArmButton_2").innerHTML = "L&#129093;";
        lArmUp = false;
        lArmMoving = true;
      }
      else {
  ...
  document.getElementById("rightArmButton_2").addEventListener('click', function() {
    dragging = false;
    if(!showing) {
      if(rArmUp) {
        document.getElementById("rightArmButton_2").innerHTML = "R&#129093;";
        rArmUp = false;
        rArmMoving = true;
      }
      else {
  ...
  document.getElementById('roboReset_2').addEventListener('click', function() {
    dragging = false;
    if(!showing) reset();
  });
  ...
  function ex3Randomize() {
    ...
    document.getElementById("randX2c").innerHTML = ex3RandomList[ex3Index].code;
    $('#replyX2c').val("");
    $('#replyX2c').val("");
    document.getElementById('replyX2c').style.backgroundColor = "#fff";
    renderMathInElement(document.body);
    ex3Target = ex3RandomList[ex3Index].val;
  }
  ex3Randomize();

  document.getElementById('refreshButton_2').addEventListener('click', function() {
    ex3Randomize();
  });
  document.getElementById('refreshButton_2').style.cursor = "pointer";
  ...
  document.getElementById("ShX2a").addEventListener('click', function() {
    if(!showing) showButtonHandler(1);
  });
  document.getElementById('ShX2b').addEventListener('click', function() {
    if(!showing) showButtonHandler(2);
  });
  document.getElementById('ShX2c').addEventListener('click', function() {
    if(!showing) showButtonHandler(3);
  });

  document.getElementById('CkX2a').addEventListener('click', function() {
    if(Math.abs(robotRotation + Math.PI/2) < .1) { // right
      $('#replyX2a').val("Correct");
      $('#readoutX2a').val("-1/2");
      document.getElementById('replyX2a').style.backgroundColor = "#8f8";
    }
    else { // wrong
      $('#replyX2a').val("Incorrect");
      $('#readoutX2a').val(Math.round(robot.rotation.y * 100 / Math.PI) / 100);
      document.getElementById('replyX2a').style.backgroundColor = "#f88";
    }
  });

  document.getElementById('CkX2b').addEventListener('click', function() {
    if(Math.abs(robotRotation - (-Math.PI)) < .1) { // right
      $('#replyX2b').val("Correct");
      $('#readoutX2b').val("-1");
      document.getElementById('replyX2b').style.backgroundColor = "#8f8";
    }
    else { // wrong
      $('#replyX2b').val("Incorrect");
      $('#readoutX2b').val(Math.round(robot.rotation.y * 100 / Math.PI) / 100);
      document.getElementById('replyX2b').style.backgroundColor = "#f88";
    }
  });

  document.getElementById('CkX2c').addEventListener('click', function() {
    if(Math.abs(robotRotation - ex3Target) < .1) { // right
      $('#replyX2c').val("Correct");
      $('#readoutX2c').val(Math.round(ex3Target * 100 / Math.PI) / 100);
      document.getElementById('replyX2c').style.backgroundColor = "#8f8";
    }
    else { // wrong
      $('#replyX2c').val("Incorrect");
      $('#readoutX2c').val(Math.round(robot.rotation.y * 100 / Math.PI) / 100);
      document.getElementById('replyX2c').style.backgroundColor = "#f88";
    }
  });
